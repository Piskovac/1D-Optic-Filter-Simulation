...
class RefractiveIndex:
    ...
    def __init__(self, databasePath=os.path.join(os.path.expanduser("~"), "refractiveindex.info-database"), auto_download=True):
        ...
        # Try new catalog format first, fallback to old library.yml
        catalog_files = ["catalog-nk.yml", "library.yml"]
        self.catalog = None

        for catalog_file in catalog_files:
            fileName = os.path.join(self.referencePath, catalog_file)
            if os.path.exists(fileName):
                try:
                    with open(fileName, "rt", encoding="utf-8") as f:
                        self.catalog = yaml.load(f, Loader=BaseLoader)
                    print(f"Loaded catalog from {catalog_file}", file=sys.stderr)
                    break
                except Exception as e:
                    print(f"Error loading {catalog_file}: {e}", file=sys.stderr)
                    continue

        if self.catalog is None:
            raise FileNotFoundError("No valid catalog file found (tried: catalog-nk.yml, library.yml)")
    ...
    def getMaterialFilename(self, shelf, book, page):
        ...
        for sh in self.catalog:
            # Handle both old format (sh['SHELF']) and new format (sh.get('SHELF'))
            shelf_id = sh.get('SHELF', '') if 'SHELF' in sh else sh.get('shelf', '')

            if shelf_id == shelf:
                for b in sh.get('content', []):
                    if 'DIVIDER' not in b:
                        # Handle both old format (b['BOOK']) and new format (b.get('BOOK'))
                        book_id = b.get('BOOK', '') if 'BOOK' in b else b.get('book', '')

                        if book_id == book:
                            for p in b.get('content', []):
                                if 'DIVIDER' not in p:
                                    # Handle both old format (p['PAGE']) and new format (p.get('PAGE'))
                                    page_id = p.get('PAGE', '') if 'PAGE' in p else p.get('page', '')
