...
class MaterialHandler:
    ...
    def serialize_material(material, selected_variant=None):
        """Convert material data to a serializable format for saving, with selected variant"""
        name, material_id, is_defect = material

        # Debug print to diagnose problematic materials
        print(f"Serializing material: {name}, Type: {type(material_id)}, Value: {material_id}")

        # Check if material_id is a number (custom material)
        ...
        # Check if material_id is a file path (browsed material)
        elif isinstance(material_id, str) and material_id.endswith('.yml'):
            # For browsed materials, extract filename and prepare for relative paths
        ...
        # Database material with selected variant
        elif selected_variant and isinstance(selected_variant, str):
            return {
                "name": name,
                "type": "database",
                "id": material_id,  # Original material ID (might be a JSON string or other format)
                "selected_variant": selected_variant,  # The actually selected variant
                "is_defect": is_defect
            }
        # Standard database material or any other string
        else:
            return {
                "name": name,
                "type": "database",
    ...
    def deserialize_material(data, project_dir=None):
        ...
        if material_type == "custom_complex":
            # Recreate complex refractive index
            n = data.get("n", 1.5)
            k = data.get("k", 0.0)
            ...
            # Handle loaded material file
            filename = data.get("filename", "")
            original_path = data.get("original_path", "")

            # If we have a project directory, use the local copy
            ...
            # Otherwise fallback to original path
                ...
                # If file is missing, return a default material
            ...
            if "selected_variant" in data and data["selected_variant"]:
                # Return the selected variant instead of the original ID
                return (data.get("name", "Unknown"), data["selected_variant"], data.get("is_defect", False))
            else:
                # Standard database material
...
class CustomMaterialDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.n_spin.setValue(1.5)  # Default to glass-like
        ...
        self.k_spin.setValue(0.0)  # Default to transparent
        ...
        # Add note about wavelength dependence
...
class MaterialTable(QTableWidget):
    ...
    def update_material_variant(self, label, variant_id):
        """Update a material's variant after selection in TMM calculation"""
        for row in range(self.rowCount()):
            if self.item(row, 0).text() == label:
                ...
                # Update the material_id data
                self.item(row, 1).setData(Qt.UserRole, variant_id)
                return True
        return False
...
class TMM_Worker(QThread):
    """Worker thread for TMM calculations to keep the UI responsive"""
...
class OpticalFilterApp(QMainWindow):
    ...
    def browse_material(self):
        ...
        try:
            ...
            if 'REFERENCES' in material_data:
                material_info['reference'] = material_data['REFERENCES']

            # Check if file contains comments
            if 'COMMENTS' in material_data:
                material_info['comments'] = material_data['COMMENTS']

            # Check if file contains data
            if 'DATA' not in material_data:
                raise ValueError("Material file does not contain DATA section")

            # Detect material type from data
            data_type = None
            ...
            # Create a temporary ID for the material based on filename
            folder_path = os.path.dirname(file_path)
            parent_folder = os.path.basename(folder_path)

            # Create dialog to finalize material details
            dialog = QDialog(self)
            ...
            # Material ID field
            # Generate a default ID from the first 3 characters of the filename
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                # Create a special structure for the material
                # In reality, we'd need to create a custom refractiveindex entry
                # For now, let's just store the file path as the material identifier
                ...
                self.statusBar().showMessage(f"Material '{material_name}' added from {os.path.basename(file_path)}",
                                             3000)
    ...
    def add_custom_material(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # Create a complex refractive index if k > 0
            refractive_index = complex(n_value, k_value) if k_value > 0 else n_value

            # Store as a direct value rather than a material ID
            # We'll need special handling when calculating with this material
    ...
    def create_material_section(self, parent_layout):
        """Create the material definition section with added browse and custom buttons"""
        ...
        # Add additional buttons for browse and custom materials
    ...
    def save_filter(self):
        """Save the current filter configuration with selected material variants"""
        ...
        try:
            ...
            # Create subdirectory for material files
            materials_dir = os.path.join(project_dir, "materials")
            os.makedirs(materials_dir, exist_ok=True)

            # Prepare material data with proper serialization
            materials_data = {}
            for label, material in self.material_table.get_materials().items():
                try:
                    # Get material data
                    # We don't need to pass a selected_variant since the material table
                    # has already been updated during calculate_tmm
                    materials_data[label] = MaterialHandler.serialize_material(material)

                    # If it's a browsed material, copy the file to the materials directory
                    ...
                    if isinstance(material_id, str) and material_id.endswith('.yml'):
                        ...
                        if os.path.exists(original_file):
                            ...
                            try:
                                # Copy the material file to the project directory
                    ...
                    # Skip this material if there's a problem
            ...
            # Save filter configuration to main file
            filter_file = os.path.join(project_dir, "filter_config.json")
            with open(filter_file, "w") as f:
                json.dump(filter_data, f, indent=2)

            # Save plot data as CSV if available
            ...
            # Add to recent files (just the main directory)
            ...
            # Show error message with traceback for debugging
            error_msg = f"Error saving filter: {str(e)}\n{traceback.format_exc()}"
            QMessageBox.critical(self, "Save Error", error_msg)

    def load_filter(self, project_path):
        """Load a filter configuration from a project directory"""
        try:
            # Check if this is a directory (new format) or file (old format)
            if os.path.isdir(project_path):
                # New format - project directory
            ...
            # Try to load plot data from CSV
            try:
    ...
    def browse_filter(self):
        ...
        # First ask whether to load a project directory or a single file
    ...
    def open_recent_file(self, index):
        ...
        if index < len(self.recent_files):
            ...
            if os.path.exists(path):
                ...
                # File or directory doesn't exist anymore
    ...
    def calculate_tmm(self):
        """Calculate and display TMM results with material variant tracking"""
        ...
        # First check material compatibility - this will also select best variants
        ...
        # Continue with calculation - material variants have already been selected
        # in the check_materials_compatibility function
        ...
        try:
            ...
            # Remove any "..." from the expanded definition if somehow present
            ...
            # Get material data - already updated with selected variants
            ...
            # Add each layer from the filter with selected material variant
            for material_id in expanded_filter:
                if material_id in materials_dict:
                    _, material_data, _ = materials_dict[material_id]
                    stack.append((material_data, default_thickness))
                else:
    ...
    def check_materials_compatibility(self):
        ...
        First selects appropriate variants before checking compatibility
        """
        # Get wavelength range
        ...
        # First, select best variants for all materials that need them
        # This is similar to what we do in calculate_tmm, but just the variant selection part
        for material_id in set(expanded_filter):  # Use set for unique materials
            if material_id in materials_dict:
                # Get current material info
                material_name, material_data, is_defect = materials_dict[material_id]

                # Skip materials that are numeric values (custom materials)
                if not isinstance(material_data, str):
                    continue

                # Skip browsed materials (they'll be checked after selection)
                if material_data.endswith('.yml'):
                    continue

                # Handle database materials that need variant selection
                if '{' in material_data:  # Check if it contains variants JSON
                    try:
                        # Parse stored variants data
                        variants_data = json.loads(material_data)
                        variants = variants_data.get("variants", [])

                        # Find best variant for current wavelength range
                        best_variant = None
                        best_coverage = 0
                        best_range = (0, 0)
                        ...
                        if best_variant:
                            # Found a good variant, update the material data
                            ...
                            # Check if it fully covers the wavelength range
                            min_range, max_range = best_range
                            if not (min_range <= start_wavelength and max_range >= end_wavelength):
                                # Only partially compatible, still warn
        ...
        # Now check all materials again, with their updated variants
        # Refresh materials dictionary after potential updates
        materials_dict = self.material_table.get_materials()
        ...
        for material_id in set(expanded_filter):
            if material_id in materials_dict:
                ...
                # Skip custom materials (numeric values)
                if not isinstance(material_data, str):
                    continue

                # Handle browsed materials
                if material_data.endswith('.yml'):
                    try:
                        # Load YAML and check wavelength range
                        import yaml
                        with open(material_data, 'r') as f:
                            yml_data = yaml.safe_load(f)

                        # Check if any data items have wavelength range info
                            ...
                            if data_item.get('type', '').startswith('tabulated'):
                                ...
                                if data_str:
                                    ...
                                    if wavelengths:
                                        ...
                                        # Check compatibility
                                        if not (min_range <= start_wavelength and max_range >= end_wavelength):
                                            incompatible_materials.append(
                                                (material_id, (min_range, max_range), None)
                                            )
                                        break

                        if not has_wavelength_data:
                            incompatible_materials.append((material_id, (0, 0), None))
                ...
                # Handle database materials with selected variants
                elif '|' in material_data:  # This is now a direct reference to a variant
                    try:
                        # Check wavelength range for the selected variant
                        min_range, max_range = self.material_api.get_wavelength_range(material_data)

                        # Check if it fully covers the wavelength range
                        if not (min_range <= start_wavelength and max_range >= end_wavelength):
                            incompatible_materials.append((material_id, (min_range, max_range), material_data))
                    except Exception as e:
